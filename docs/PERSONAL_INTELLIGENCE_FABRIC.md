...
right to pause or stop remain with the user.
...
**IMPLEMENTED FOUNDATION:** local runtime discovery recognizes LM Studio, Ollama, and llama.cpp
candidates without opening sockets. Verified model observations can be converted into conservative
context, parallelism, GPU-offload, reasoning, role, capability, and health-check profiles.
...
visual review, and synthesis. The deterministic planner honors capabilities, per-wave concurrency,
latency, quality, cost, stable tie-breaking, and reviewer independence when capacity permits.
...
## Merge path for the newer local implementation

The public foundations are intentionally additive and low-collision. When the newer local Fabric is
uploaded, reconcile in this order:
