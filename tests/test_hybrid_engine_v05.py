...
def test_privacy_sensitive_work_is_local_first_only_when_capabilities_fit() -> None:
    ...
    assert not local.dispatch_blocked
    assert not local.remote_fallback_allowed
    assert not local.bridge_enabled
    assert not insufficient.local_first
    assert insufficient.dispatch_blocked
    assert not insufficient.remote_fallback_allowed
...
def test_simple_latency_sensitive_request_uses_cheap_first_escalation() -> None:
    ...
    assert plan.remote_fallback_allowed


def test_hybrid_uses_canonical_capabilities_and_leaves_worker_selection_pending() -> None:
    coding = HybridEngine().plan(HybridRequest("code", HybridWorkloadKind.CODING))
    protocol = coding.to_protocol()

    assert coding.blueprint.stages[0].required_capabilities == frozenset({"edit-code"})
    assert protocol["blueprint"]["workerSelectionAuthority"] == "deterministicScheduler"
    assert any("scheduler selects each Worker" in fact for fact in coding.facts)
