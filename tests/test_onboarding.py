...
def test_hybrid_workstation_is_ready_without_performing_mutations() -> None:
    ...
    assert protocol["automaticActions"] == []
    assert protocol["bootstrapPlanSchemaVersion"] == 2
    assert all(not step.mutating for step in guide.safe_steps)


def test_cluster_node_preset_keeps_private_transport_mutation_approval_gated() -> None:
    ...
    assert any(step.step_id == "configure-private-transport" for step in guide.approval_steps)
    assert any(step.step_id == "verify-private-transport" for step in guide.safe_steps)
...
def test_unsupported_platform_is_blocked() -> None:
    ...
    platform_step = next(
        step for step in guide.safe_steps if step.step_id == "validate-platform"
    )
    assert platform_step.status == "blocked"
